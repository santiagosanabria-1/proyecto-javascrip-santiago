...
let reserva_promo = null; // { code, discount, active } una vez validado -- null si no hay ninguno aplicado
 ...
 * el número que realmente se cobra se recalcula desde cero en confirmación
...
async function handleConfirm(triggerBtn, mode) {
    ...
    try {
        ...
        // leídos de nuevo. Sin esto, cambiar `reserva_flow.total` (o el
        // `type` de un asiento) antes de confirmar dejaba comprar cualquier
        // cantidad de entradas -- o asientos VIP al precio de standard --
        // por el precio que el cliente quisiera (encontrado y verificado en
        // la auditoría).
        ...
        // El código promocional también se revalida en este momento (no se
        // confía en el objeto que quedó en memoria desde que se aplicó, por
        // si mientras tanto se desactivó).
        let discount = 0;
        let promoCode = null;
        if (reserva_promo) {
...
function showSuccessView(record, mode) {
    ...
    // guardaron (recalculados en confirmación), nunca los que traía
